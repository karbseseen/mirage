...
import com.frostwire.jlibtorrent.swig.{info_hash_t, torrent_flags_t}


private[torrent] opaque type Hash = Sha1Hash | Sha256Hash

private[torrent] object Hash:
...
extension (handle: TorrentHandle) private[torrent] def isPaused = handle.flags.and_(TorrentFlags.PAUSED).nonZero
